  ...
  switch (queryType) {
    case "byGenre":
      for (let i = 0; i < responseFromApi.works.length; i++) {
        let newBook = generateNewBook(
          responseFromApi.works[i].title,
          responseFromApi.works[i].authors[0].name,
          responseFromApi.works[i].cover_id,
          responseFromApi.works[i].key
        );
    ...
    case "byAuthor":
      for (let i = 0; i < responseFromApi.entries.length; i++) {
        let newBook = generateNewBook(
          responseFromApi.entries[i].title,
          author,
          selectCover(responseFromApi.entries[i].covers),
          responseFromApi.entries[i].key
        );
    ...
    case "byTitle":
      ...
      for (let i = 0; i < responseFromApi.docs.length; i++) {
        let newBook = generateNewBook(
          responseFromApi.docs[i].title,
          responseFromApi.docs[i].author_name,
          responseFromApi.docs[i].cover_i,
          responseFromApi.docs[i].key
        );
